...
#define N 10//the number of values in the data
...
if (rank == MASTER)
{
  ...
  MPI_Scatter(init_sample, partition, MPI_C_DOUBLE_COMPLEX, sample, partition, MPI_C_DOUBLE_COMPLEX, MASTER, MPI_COMM_WORLD);
	...
	for(K=0; K<N; K++)
	{
		...
		double complex newtotal;
   MPI_Reduce(totalresult, newtotal, size, MPI_C_DOUBLE_COMPLEX, MPI_SUM, MASTER, MPI_COMM_WORLD);
	 	if(rank == MASTER)
		{
